...
// pub fn mint_nft(ctx: Context<MintNft>, name: String, 
//     hp: u16, 
//     atk: u16, 
//     def: u16, 
//     metadata_uri: String // IPFS/Arweave link for actions/results
// ) -> Result<()> {
...
//             ctx.accounts.mint.key,
//             ctx.accounts.nft_authority.to_account_info().key,
//             spl_token_metadata_interface::state::Field::Key("hp".to_string()), 
//             hp.to_string() // Store HP as a string
//         ),
//         &[
//             ctx.accounts.mint.to_account_info().clone(),
//             ctx.accounts.nft_authority.to_account_info().clone(),
...
//             ctx.accounts.mint.key,
//             ctx.accounts.nft_authority.to_account_info().key,
//             spl_token_metadata_interface::state::Field::Key("atk".to_string()), 
//             atk.to_string()
//         ),
//         &[
//             ctx.accounts.mint.to_account_info().clone(),
...
//     // Store DEF in NFT metadata
//     invoke_signed(
//         &spl_token_metadata_interface::instruction::update_field(
//             &spl_token_2022::id(),
//             ctx.accounts.mint.key,
//             ctx.accounts.nft_authority.to_account_info().key,
//             spl_token_metadata_interface::state::Field::Key("def".to_string()), 
//             def.to_string()
//         ),
//         &[
//             ctx.accounts.mint.to_account_info().clone(),
...
#[program]
mod hack_illinois_25 {
    use super::*;

    pub fn mint_nft(ctx: Context<MintNft>, name: String, 
        hp: u16, atk: u16, def: u16, metadata_uri: String) -> Result<()> {
        ...
        // This is the space required for the metadata account.
        // We put the meta data into the mint account at the end so we
        // don't need to create an additional account.
        let meta_data_space = 250;
